...
select nombre_profesor,fecha_nacimiento_profesor from ciclos_profesor p where p.departamento in (select departamento from ciclos_profesor where nombre_profesor='Marta Vidal');
...
select nombre_profesor from ciclos_profesor where departamento in (select codigo_departamento from CICLOS_departamento where jefe_departamento='Carlos Martín Ruiz' );
...
select nombreArtistico, nombreReal, nom_ciudad from lista_artistaind ai, lista_artista a where ai.cod=a.cod and a.nom_pais='Estados Unidos'; 
/*2. Muestra la posición, el nombre del artista y el nombre de la canción de la lista 7.*/
select posicion, autor, titulo from listas_lista l, LISTAS_POSICION_LISTA p, Listas_cancion c where l.cod =p.COD_LISTA and p.cod=7 and c.cod=p.COD_CANCION;
...
select NOMBREARTISTICO, fnac, fmuerte from LISTAS_ARTISTA a, lista_artistaind ai where ai.cod=a.cod and to_char(fnac, 'yyyy')>'1970';
...
select nombreArtistico, FDISOLUCION from LISTAS_GRUPO g, LISTAS_ARTISTA a where a.cod=g.cod and FCREACION>(select fmuerte from lista_artista a, LISTAS_ARTISTAIND ai where a.nombreArtistico='Elvis Presley' and a.cod=ai.cod);
